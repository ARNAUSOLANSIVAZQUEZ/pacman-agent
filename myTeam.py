...
class StallReflexAgent(ReflexCaptureAgent):
    def get_features(self, game_state, action):
        ...
        my_food = self.get_food_you_are_defending(game_state).as_list()
        ...
        if len(food_list) > 0:
            ...
            features['objective_distance'] = min_distance / min_enemy_distance
        if len(food_list) < 20:
            if not(my_state.is_pacman): features['on_defense'] = 1
            if len(invaders) > 0:
    ...
    def get_weights(self, game_state, action):
        return {'successor_score': 100, 'on_defense': 100,
                'objective_distance': -10, 'num_invaders': -1000}
